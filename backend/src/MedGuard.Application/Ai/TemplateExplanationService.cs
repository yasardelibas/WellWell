        ...
        switch (finding.Type)
        {
            case SafetyFindingType.DuplicateActiveIngredient:
                ...
                if (tr)
                {
                    ...
                    {
                        >= 2 => $"MedGuard {string.Join(" ve ", products)} etiketlerini okudu ve her birinin {ingredient} maddesini içerdiğini tespit etti. ",
                        _ => $"MedGuard, {ingredient} maddesinin kayıtlı ilaçlarınız arasında birden fazla kez listelendiğini tespit etti. "
                    });
                    ...
                    builder.Append("MedGuard size bir şey değiştirmenizi söylemiyor. ");
            ...
            case SafetyFindingType.UnverifiedMedication:
                if (tr)
                {
                    builder.Append("MedGuard bu ilacı güvenilir bir ilaç veritabanıyla eşleştiremedi. ");
            ...
            case SafetyFindingType.InteractionCheckUnavailable:
                if (tr)
                {
                    builder.Append("Herhangi bir etkileşim veri kaynağı kullanılamadığı için MedGuard ilaçlarınız arasında etkileşim kontrolü yapmadı. ");
            ...
            default:
                if (tr)
                {
                    builder.Append("MedGuard bu öğeyi listenizde kayıtlı ilaçlara dayanarak işaretledi. ");
